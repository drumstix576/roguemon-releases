local self = {
    ...
    stats = {},
    pendingStatId = nil,
    selectionWatchLabel = "Roguemon:NatureMintNerfWatch",
}
...
function self.selectStat(entry)
    ...
    Roguemon.ScreenManager.queueDeferredSubmission(self, function()
        return self.PrizeManager.submitNatureMintNerf(entry.id)
    end)
    ...
    Program.addFrameCounter(self.selectionWatchLabel, 10, function()
        ...
        if queueCount == 0 or headTask ~= TASK_ID then
            ...
            elseif Program.currentScreen == Roguemon.Screens.NatureMintNerfScreen then
